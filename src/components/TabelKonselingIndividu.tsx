...
import { KonselingIndividu } from '../types';
import { downloadKonselingIndividuWord } from '../lib/wordExporter';
import {
  FileText,
  Search,
  Filter,
...
interface TabelKonselingIndividuProps {
  items: KonselingIndividu[];
  onEdit: (item: KonselingIndividu) => void;
  onDelete: (id: string) => Promise<void>;
  onPrintItem?: (item: KonselingIndividu) => void;
  onPrintRekap?: () => void;
}

export const TabelKonselingIndividu: React.FC<TabelKonselingIndividuProps> = ({
  items,
  onEdit,
  onDelete,
  onPrintItem,
  ...
  const [selectedDetail, setSelectedDetail] = useState<KonselingIndividu | null>(null);
  ...
  const handleDeleteConfirm = async (id: string) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus data rencana konseling individu ini?')) {
      setDeletingId(id);
      try {
        await onDelete(id);
      } finally {
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-200 pb-5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-indigo-800 font-semibold flex items-center gap-1">
                ...
                Terdata {filtered.length} Rencana Konseling Individu
              </span>
            </div>
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight mt-0.5">
              REKAP DATA RENCANA KONSELING INDIVIDU
            </h3>
        ...
        {onPrintRekap && (
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-700 hover:bg-indigo-800 text-white text-xs sm:text-sm font-bold rounded-xl shadow-md transition-all self-start lg:self-auto"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200">
        <div className="md:col-span-2 relative">
          ...
          <input
            ...
            placeholder="Cari nama siswa, kelas, topik permasalahan, atau pendekatan konseling..."
            className="w-full bg-white border border-slate-300 rounded-xl pl-10 pr-3.5 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
          />
        </div>
        ...
        <div className="relative">
          ...
          <select
            ...
            className="w-full bg-white border border-slate-300 rounded-xl pl-10 pr-3.5 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all cursor-pointer"
          >
              ...
              <tr>
                <td colSpan={8} className="p-10 text-center text-slate-400 italic bg-slate-50/50">
                  <div className="max-w-xs mx-auto space-y-2">
                    ...
                    <p className="font-semibold text-sm">Tidak ada data rencana konseling individu ditemukan.</p>
              ...
              filtered.map((item, idx) => (
                <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="p-3 font-medium">
                    ...
                    <div className="text-[10px] text-indigo-700 font-mono font-bold mt-0.5">{item.waktu || '08:00 WIB'}</div>
                  </td>

                  {/* Kelas & Siswa */}
                  <td className="p-3">
                    <span className="inline-block bg-indigo-100 text-indigo-900 font-extrabold text-[10px] px-2 py-0.5 rounded mb-1">
                      Kelas {item.kelas}
                    </span>
                  ...
                  <td className="p-3 text-center">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {/* Download Word Doc */}
                      <button
                        onClick={() => downloadKonselingIndividuWord(item)}
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 space-y-5 animate-in fade-in zoom-in duration-150">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="bg-indigo-100 text-indigo-900 text-[10px] font-black px-2 py-0.5 rounded uppercase">
                    DETAIL RENCANA KONSELING INDIVIDU
                  </span>
            ...
            <div className="space-y-4 text-xs leading-relaxed text-slate-700">
              <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
                ...
                <div>
                  ...
                  <span className="font-extrabold text-indigo-800 text-sm">Kelas {selectedDetail.kelas}</span>
                </div>
                <div>
              ...
              <div className="space-y-3">
                <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200">
                  <span className="font-extrabold text-slate-900 block mb-1 uppercase tracking-wider text-[11px] text-indigo-800">
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-200 pt-4">
              <button
                onClick={() => downloadKonselingIndividuWord(selectedDetail)}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition-colors flex items-center gap-1.5"
              >
                <FileDown className="w-3.5 h-3.5" />
